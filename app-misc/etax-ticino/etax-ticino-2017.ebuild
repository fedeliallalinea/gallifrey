...
SRC_URI="
	amd64? ( https://www3.ti.ch/DFE/DC/${PN/-ticino/}/${PV}/${MY_PV}-64bit.sh )
	x86? ( https://www3.ti.ch/DFE/DC/${PN/-ticino/}/${PV}/${MY_PV}-32bit.sh )
"
DEPEND="virtual/jre:*"
RDEPEND="${DEPEND}"
...
src_prepare() {
	default
	sed \
		-e "s@/bin/java\" -Dinstall4j.jvmDir=\"\$app_java_home\"@/bin/java\" -Duser.home="${WORKDIR}" -Dinstall4j.jvmDir="${WORKDIR}" -Djava.util.prefs.userRoot="${WORKDIR}"@" \
		-i "${MY_PV}.sh" \
		|| die "failed to set userHome and jvmDir where JAVA .systemPrefs can be found"
	sh "${S}/${MY_PV}".sh -q -VcreateDesktopLinkAction$Boolean=false -dir "${S}" > /dev/null || die "unpack failed"
}
