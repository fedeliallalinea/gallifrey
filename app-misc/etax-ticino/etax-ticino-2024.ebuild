...
src_prepare() {
	default
	sed \
		-e "s@/bin/java\"@/bin/java\" -Duser.home="${WORKDIR}" -Dinstall4j.jvmDir="${WORKDIR}" -Djava.util.prefs.systemRoot="${WORKDIR}"@" \
		-i "${MY_PV}.sh" \
	...
	sed -i "s/-XX:PermSize=256m/-XX:MetaspaceSize=256m/g" "eTax.ticino PF ${PV}" || die
	sed -i "s/-XX:MaxPermSize=256m/-XX:MaxMetaspaceSize=256m/g" "eTax.ticino PF ${PV}" || die
	sed -i "s/\"-XX:+AggressiveOpts\"//g" "eTax.ticino PF ${PV}" || die
	sed -i "s/\"-XX:+UseParallelOldGC\"//g" "eTax.ticino PF ${PV}" || die
}
